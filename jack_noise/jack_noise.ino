...
//    uint8_t yoffset = scale * ledsArray[i].y;
    if (isvalueinarray(i+1, skipLeds, NUM_SKIP_LEDS)) {
      continue;
    }

    uint16_t id = i;    
    uint16_t xoffset = scale * ledsArray[i][0];    
    uint16_t yoffset = scale * ledsArray[i][1];
