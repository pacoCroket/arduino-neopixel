...
void mapCoordToColor() {
  ...
  for (int i = 0; i < NUM_LEDS; i++) {
    ...
    uint16_t zoffset = ledsArray[i][2] * scale;
    
    uint16_t index = inoise16(x + xoffset, y + yoffset, z + zoffset);
//
...
//    uint8_t index = inoise8(x + xoffset, y + yoffset z);
    uint16_t bri = inoise16(x + yoffset, y + xoffset, z + zoffset); // another random point for brightness

    if( dataSmoothing ) {
      uint16_t olddata = inoise16(x + xoffset - speed / 8,y + yoffset + speed / 16,z-speed);
      uint16_t newdata = scale16( olddata, dataSmoothing) + scale16( index, 256 - dataSmoothing);
      index = newdata;
    }
...
// etc
#define HOLD_PALETTES_X_TIMES_AS_LONG 7
...
void ChangePaletteAndSettingsPeriodically()
{
  ...
  if( lastSecond != secondHand) {
    ...
    if( secondHand == 5)  { SetupBlackAndWhiteStripedPalette();       speed = 9 * speedFactor; scale = 5 * scaleFactor; colorLoop = 1; }
    if( secondHand ==  10)  { SetupPurpleAndGreenPalette();             speed = 4 * speedFactor; scale = 5 * scaleFactor; colorLoop = 1; }
//    if( secondHand == 15)  { currentPalette = ForestColors_p;          speed =  3; scale = 8 * scaleFactor; colorLoop = 0; }
    ...
    if( secondHand == 40)  { SetupRandomPalette();                     speed = 4 * speedFactor; scale = 4 * scaleFactor; colorLoop = 1; }
